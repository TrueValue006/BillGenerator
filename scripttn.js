...
function determineBranchType(branchName) {
    let branchType = ''; // Default to UBI if no hyphen is found
  
    // Check if there's a hyphen in the branch name
    if (branchName.includes('-')) {
      const parts = branchName.split('-');
      branchName = parts[0].trim();      // Take the part before the hyphen as the branchName
      branchType = parts[1].trim();      // Take the part after the hyphen as the branchType
    }
...
function displayBills(data) {
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      const Region = row[headers.indexOf('Region')]; 
    ...
    <div class="container-b">
        <div class="intro">
            ...
            <div class="report">
                    <h3 class="cpv" style="font-weight:bold;">CONTACT POINT VERIFICATION / DUE DELIGENCE REPORT</h3> 
                         ...
                         Ph.No: 0427 – 2468093, E-Mail: truevalue.salem@gmail.com  <br>          
                                <div><h3 class="cpv" style="display: inline;">GST No :</h3><h4 style="display: inline; font-size: 20px;"> 33AAGCT5341Q1ZX</h4></div>
                            </p>
            </div>
        </div>
        ...
        <div class="invoice-details">
            <table>
                <tr>
                    ...
                    <td style="padding-left: 69px !important; width:70px;"><p style="font-size:20px;">Date&nbsp;:</p></td>
                ...
                <tr>
                    ...
                    <td style="padding-left: 50px !important;"><p style="width: 100px !important; font-size:20px;">Region :</p></td>
                    <td ><span style="font-size:20px;">${Region}</span></td>
            ...
            <table>
                <thead style="border-top: 3px solid black;border-bottom: 3px solid black;" >
                    <tr style="font-size: 20px;">
                        <th style="width: 60px !important; border-right:  3px solid black; ">S.No</th>
                        <th style="width: 260px; border-right:  3px solid black;">Customer Name</th> 
                        <th style="border-right:3px solid black;">Verified For</th>
                        <th>Taxable Amount</th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
                        <td style="width: 60px; border-right:3px solid black; font-size: 20px; font-weight: bold; text-align: center;">1</td>
                        <td style="width: 260px; border-right:  3px solid black; font-size: 20px; font-weight: bold; padding-left:5px !important;">${finalCustomerName}</td>
                        <td style="border-right:3px solid black; font-size: 20px; text-align: center; font-weight:bold;">${row[headers.indexOf('Type of Loan')]}</td>
                        <td style="font-size: 20px; text-align: center;"> ${finalAmount|| ''}</td>

                    </tr>
                    <tr>
                    <td style="width: 60px; border-right:3px solid black; font-size: 20px; font-weight: bold; text-align: center;"></td>
                    <td style="width: 260px; border-right: 3px solid black; font-size: 20px; font-weight: bold; padding-left:5px !important;">${city1 ?? ''}</td>
                    <td style="border-right:3px solid black; font-size: 20px; text-align: center;"></td>
                    <td style="font-size: 20px; text-align: center;">${charge1 ?? ''}</td>
                     </tr>
                    <tr>
                        <td style="width: 60px; border-right:3px solid black; font-size: 20px; font-weight: bold; text-align: center;"></td>
                        <td style="width: 260px; border-right: 3px solid black; font-size: 20px; font-weight: bold; padding-left:5px !important;">${city2 ?? ''}</td>
                        <td style="border-right:3px solid black; font-size: 20px; text-align: center;"></td>
                        <td style="font-size: 20px; text-align: center;">${charge2 ?? ''}</td>
                    </tr>
                    <tr style="height:80px;">
                     <td style="width: 60px; border-right:3px solid black; font-size: 20px; font-weight: bold; text-align: center;"></td>
                        <td style="width: 260px; border-right: 3px solid black; font-size: 20px; font-weight: bold;"></td>
                        <td style="border-right:3px solid black; font-size: 20px; text-align: center;"></td>
                        <td style="font-size: 20px; text-align: center;"></td>
                    </tr>
                    <tr style="border-top: 3px solid black;">
                        <td style="width: 60px; border-right:3px solid black;"></td>
                        <td style="width: 90px; border-right:3px solid black; text-align: center;font-size: 20px;" colspan="2" id="nested-data">
                            <table id="nested-table">
                                <tr id="nested-row">
                                    <td id="nested-data" style="font-size: 20px;">SGST @9%</td>
                                </tr>
                                <tr id="nested-row" >
                                    <td id="nested-data" style="font-size: 20px;">CGST @9%</td>
                                </tr>
                            </table>
                        </td>
                        <!-- <td style="width: 90px; border-right:3px solid black;"></td> -->
                        <td style="text-align: center;font-size: 18px;" id="nested-data">
                            <table id="nested-table">
                                <tr id="nested-row" style="border-bottom:2px solid black;">
                                    <td id="nested-data">${row[headers.indexOf('GST')]/2}</td>
                                </tr>
                                <tr id="nested-row">
                                    <td id="nested-data">${row[headers.indexOf('GST')]/2}</td>
                                </tr>
                    ...
                    <tr style="border-top: 3px solid black;" id="nested-data">
                        <td colspan="3" style="text-align: center; font-size: 21px; font-weight: bold; border-right: 3px solid black;" id="nested-data">Total Amount</td>
                        <td style="text-align: center; font-size: 21px; font-weight: bold;" id="nested-data"> &#8377;${formattedTotalAmount}</td>
                    </tr>
                    <tr style="border-top: 3px solid black;" id="nested-data">
                      <!--   <td id="nested-data" style="font-size: 20px; font-weight: bold;padding-left: 5px !important;">In words:</td>-->
                        <td colspan="4" style=" font-size: 18px; font-weight: bold; text-transform: capitalize; padding-left:5px !important;" id="nested-data">In Words: <span style="margin-left:30px !important; font-size:20px;"> Rupees ${totalAmountInWords} only </span></td>
                    </tr>    
                </tbody>
            </table>
        </div>
